...
class DistributionNode:
    def __init__(self, channel_id="neuron_buster"):
        ...
        # Load API keys from Firebase via FirebaseDBManager
        # To avoid circular imports, import here
        from modules.firebase_db import FirebaseDBManager
        self.db = FirebaseDBManager()
        self.config = {}
        ch_data = self.db.get_channel_config(self.channel_id)
        if ch_data and ch_data.get('api_keys_json'):
            try:
                self.config = json.loads(ch_data['api_keys_json'])
            except:
                pass
    ...
    def upload_to_instagram(self, video_path, caption="", pinned_comment=""):
        ...
        if not username or not password:
            print("❌ Instagram credentials missing in Channel Settings UI.")
            return False
